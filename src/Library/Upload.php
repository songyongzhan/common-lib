...
class Upload extends SplFileObject
{
    ...
    private function checkImg()
    {
        ...
        return !in_array($extension,
                ['gif', 'jpg', 'jpeg', 'bmp', 'png', 'swf']) || in_array($this->getImageType($this->filename),
                [1, 2, 3, 4, 6, 13]);
    }
    ...
    protected function buildSaveName($saveName)
    {
        ...
        if (true === $saveName) {
            if ($this->rule instanceof \Closure) {
                ...
                switch ($this->rule) {
                    case 'date':
                        $saveName = date('Ymd') . $this->ds . md5(microtime(true));
                        break;
                    default:
                        if (in_array($this->rule, hash_algos())) {
                            $hash = $this->hash($this->rule);
                            $saveName = substr($hash, 0, 2) . $this->ds . substr($hash, 2);
